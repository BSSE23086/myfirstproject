...
from .models import Room, road, Topic, Message, User
...
def createRoom(request):
    ...
    if request.method == 'POST':
        topic_name = request.POST.get('topic')  # Fixed: .POST capital
        topic, created = Topic.objects.get_or_create(name=topic_name) # Fixed: .objects added
        
        Room.objects.create( # Fixed: Capitalized Room and .objects
            host = request.user,
            topic = topic,
...
def updateUser(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('user-profile', pk=user.id) # Fixed: added return statement
